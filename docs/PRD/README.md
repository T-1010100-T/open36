...
基于业务功能和数据模型的内聚性，系统划分为以下 **8 个核心后端模块**：
...
| **M8**   | AI智能服务模块 | 管理员AI助手、自动发帖、智能出题     | Conversation, Task, Agent   |
...
- **M3 内容管理模块**：被 M4、M6、M8 依赖（获取帖子信息、创建帖子）
- **M5 板块管理模块**：被 M8 依赖（获取板块列表、匹配板块名）
- **M7 文件存储模块**：被 M2、M3、M4、M5 依赖（图片上传）
- **M8 AI智能服务模块**：调用 M3/M5 内部API（发帖、板块）和 HOJ API（出题），仅管理员可访问
...
7. **M6 搜索与索引模块** - 全文搜索
8. **M8 AI智能服务模块** - 管理员AI助手、自动发帖、智能出题
9. **前端管理后台** - 统一管理界面（调用各模块接口）
...
8. ✅ **M8-AI智能服务模块.md** - 管理员AI助手、LangGraph多Agent、自动发帖与出题

> 📌 **管理后台说明**：管理后台由前端实现，通过调用各模块提供的管理员接口完成管理功能，无需独立的后端服务模块。
> 📌 **M8说明**：M8是AI编排服务，不直接操作数据库，通过调用M3/M5/HOJ的内部API完成任务。
