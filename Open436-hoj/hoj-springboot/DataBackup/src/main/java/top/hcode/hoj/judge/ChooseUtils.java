...
public class ChooseUtils {
    ...
    public JudgeServer chooseServer(Boolean isRemote) {
        ...
        if (instances.size() > 0) {
            ...
            // dev 环境 Nacos discovery 禁用时，直接从数据库获取已注册的判题机
            QueryWrapper<JudgeServer> fallbackWrapper = new QueryWrapper<>();
            fallbackWrapper.eq("is_remote", isRemote).eq("status", 0);
            List<JudgeServer> fallbackList = judgeServerEntityService.list(fallbackWrapper);
            for (JudgeServer js : fallbackList) {
                keyList.add(js.getUrl());
            }
        }
        if (keyList.isEmpty()) {
            return null;
