...
public class ConfigManager {
    ...
    public JSONObject getServiceInfo() {
        ...
        // 获取nacos中心配置所在的机器环境（dev环境可能未启用Nacos，容错处理）
        try {
            if (StringUtils.hasText(NACOS_URL)) {
                String response = restTemplate.getForObject(NACOS_URL + "/nacos/v1/ns/operator/metrics", String.class);
                result.put("nacos", JSONUtil.parseObj(response));
            } else {
                result.put("nacos", new JSONObject());
            }
        } catch (Exception e) {
            log.warn("[Admin Dashboard] 获取Nacos metrics失败: {}", e.getMessage());
            result.put("nacos", new JSONObject());
        }

        // 获取当前数据后台所在机器环境（OSHI系统调用可能失败，容错处理）
        try {
            ...
            log.warn("[Admin Dashboard] 获取系统资源信息失败: {}", e.getMessage());
            result.put("backupCores", 0);
            result.put("backupPercentCpuLoad", "0.00%");
            result.put("backupPercentMemoryLoad", "0.00%");
        }
        return result;
    }
