...
export default {
  ...
  mounted() {
    // 处理从 Open436-Admin 跳转过来的 SSO 自动登录（跨端口 localStorage 不共享，通过 URL 参数传递）
    const urlParams = new URLSearchParams(window.location.search);
    const hojToken = urlParams.get('hoj_token');
    const ssoUsername = urlParams.get('username');
    const ssoRole = urlParams.get('role');
    if (hojToken) {
      localStorage.setItem('token', hojToken);
      const userInfo = {
        username: ssoUsername || '',
        roleList: ssoRole ? [ssoRole] : ['admin']
      };
      localStorage.setItem('userInfo', JSON.stringify(userInfo));
      this.$store.commit('changeUserInfo', { userInfo });
      this.$store.commit('changeUserToken', hojToken);
      // 清理 URL 参数，避免刷新时重复处理
    ...
    // 若已存在 Open436 同步的 token 且 userInfo 已设置，自动进入管理后台
    const token = localStorage.getItem('token');
    let userInfo = null;
    // 优先从 localStorage 读取（SSO 场景）
    const userInfoStr = localStorage.getItem('userInfo');
    if (userInfoStr) {
      try { userInfo = JSON.parse(userInfoStr); } catch (e) {}
    }
    // 其次从 Vuex 读取（HOJ 直接登录场景，页面刷新后 store 从 storage 恢复）
    if (!userInfo && this.$store.getters.userInfo) {
      userInfo = this.$store.getters.userInfo;
    }
    if (token && userInfo && userInfo.roleList && userInfo.roleList.length > 0) {
      this.$store.commit('changeUserInfo', { userInfo });
      this.$store.commit('changeUserToken', token);
      this.$router.push({ name: 'admin-dashboard' });
    }
  },
  methods: {
